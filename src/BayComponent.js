export default class BayComponent extends HTMLElement {
    ...
    listeners;
    script;
    // When true, component data will be applied
    initialized = false;

    constructor(template, style, name, listeners, script) {
        super();
        console.log(template, style, name, listeners, script);
        if(!template && !style && !name && !listeners && !script)
            return;
